...
select Z.num_zona, Z.nom_massa, Z.municipi from Zones Z
where (Z.num_zona, Z.nom_massa) in (select C.num_zona, C.nom_massa from Captures C
where C.long_min<20)
and (Z.num_zona, Z.nom_massa) in (select P.num_zona, P.nom_massa from Permisos P
group by P.num_zona, P.nom_massa having count(P.data_vigencia) >= 3);